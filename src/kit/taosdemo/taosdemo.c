...
static struct argp_option options[] = {
  {0, 'h', "host",                     0, "The host to connect to TSDB. Default is localhost.",                                                           0},
  {0, 'p', "port",                     0, "The TCP/IP port number to use for the connection. Default is 0.",                                                  1},
  {0, 'u', "user",                     0, "The TSDB user name to use when connecting to the server. Default is 'root'.",                                  2},
  {0, 'a', "password",                 0, "The password to use when connecting to the server. Default is 'steerinfo'.",                                        3},
...
int main(int argc, char *argv[]) {
  ...
  if (taos == NULL) {
    fprintf(stderr, "Failed to connect to TSDB, reason:%s\n", taos_errstr(taos));
    taos_close(taos);
    return 1;
  }
